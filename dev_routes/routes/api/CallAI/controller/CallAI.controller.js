...
import old_Transcript from "../../../../models/old_Transcript.model.js";
import { speech_cofig } from "../proccess/assets/chatgptconfig.js";
...
export const fromListener = async (req, res) => {
  try {
    ...
    let agent = await Agents.findOne({
      ...
      include: [
        { model: AgentLists, required: true },
        {
          model: Groups,
          include: {
            model: GroupServiceConfig,
...
export const generatePrompt = async (req, res) => {
  try {
    ...
    const { intent, type, organization_id, metricsType, api_type, changeType } =
      req.body;
    ...
    let changeReplace =
      type == undefined
        ...
        ? metricsType
        : intent;
    let a = prompt.replace(toChange, changeReplace);
    let chatgpt = prompt_suggestion(a);
...
export const getGeneratePrompt = async (req, res) => {
  try {
    const { id } = req.query;
    let a = await promptGenDetails.findOne({
      where: {
        code: id,
      },
    });
    let toReturn = a === null ? false : changeToJson(a);
    if (toReturn.status === "Error") throw new Error(toReturn.results);
...
export const reRunTranscriptSTTController = async (req, res) => {
  try {
    let data = {
      headers: {
        Authorization: "Bearer " + process.env.OPEN_AIAPI_KEY,
    ...
    if (getTranscript === null) throw new Error("Transcript Not Found");
    let toJson = getTranscript.toJSON();
    let queue_id = toJson.Query.queue_id;
    let getOldTranscript = await old_Transcript.findOne({
      where: { old_id: id },
    });
    if (getOldTranscript !== null)
      throw new Error("this Transcript is already re run");
    // get call file
    let audiPath = toJson.StoredSpeeches[2].path;
    let path_ = await new URL(
      "../../../../upload/" + audiPath,
      import.meta.url
    );
    ...
    let speechConfig = speech_cofig(base64Data);
    data.data.push(speechConfig);
    // save old transcript here
    // get details of the org first

    let org = new getOrg();
    await org.start(toJson.Group.organization_id);
    let apikey = org.getApiByCallback(0);
    // execute the file
    // let er = new executeRequest();
    // await er.start(data, apikey);
    // let response = await er.start_call(apikey);
    ...
    let copyOfOriginal = toJson;
    // remove agent field and group field
    delete copyOfOriginal["Agent"];
    delete copyOfOriginal["Group"];
    // then save the transcript here

    // old_Transcript.create(copyOfOriginal);
    res.send(changeSend(toJson));
  } catch (err) {
    console.log(err);
    throw err;
  }
