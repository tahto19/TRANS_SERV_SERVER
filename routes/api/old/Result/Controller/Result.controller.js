  ...
  try {
    ...
    let r = await Transcripts.findAll({
      ...
      include: [
        ...
        { model: Agents, where: queryAgent, required: true },
        {
          required: true,
          model: Notes,
        },
  ...
  try {
    ...
    let r = await Transcripts.findAll({
      ...
      include: [
        ...
        { model: Agents, required: true, where: queryAgent },
        {
          required: true,
          model: IntentResult,
      ...
      // group: ["IntentResults.main_intent.intent_name", "Agent.id"],
    });
...
export const getCSATversion2 = async (req, res) => {
  try {
    ...
    const getIntents = await Transcripts.findAll({
      ...
      group: ["IntentResults.main_intent.conn"],
    });
  ...
  try {
    ...
    let a = await Transcripts.findAll({
      include: [
        ...
        { model: Agents, required: true, where: queryForAgent },
        {
          required: true,
          model: IntentResult,
          // attributes: [],
      ...
      attributes: [
        ...
        [
          Sequelize.fn(
            "CONCAT",
            Sequelize.literal(
              "CASE WHEN `IntentResults->main_intent->Intent`.`active` = 1 THEN '' ELSE 'old ' END"
            ),
...
export const getCSATPerKPI = async (req, res) => {
  try {
      ...
      attributes: [
        ...
        [
          Sequelize.fn(
            "CONCAT",
            Sequelize.literal(
              "CASE WHEN `Transcript->IntentResults->main_intent->Intent`.`active` = 1 THEN '' ELSE 'old ' END"
            ),
            Sequelize.col("Transcript.IntentResults.main_intent.Intent.intent")
          ),
          "intent_name",
        ],
    ...
    res.send(changeSend({ a, asdfa: "sadf" }));
  } catch (err) {
    console.log(err);
    throw err;
  }
...
export const getCSATAgentScoreCard = async (req, res) => {
  try {
    ...
    let a = await KpiAnylsis.findAll({
              ...
              include: [
                {
                  ...
                  include: {
                    ...
                    model: Intents,
                    where: {
                      active: !intent.includes("Old"),
                      intent: getRealIntent_name,
                    },
                  },
                },
              ],
      ...
      group: ["kpi_name", "Agent_id", "intent_name"],
    });
