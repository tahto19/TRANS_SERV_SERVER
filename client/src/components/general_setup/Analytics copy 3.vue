                  ...
                  >
                    <v-expansion-panel-title class="py-1">
                      ...
                      >
                        ...
                        <div>
                          <div
                            class="d-flex flex-md-row flex-wrap justify-center align-center flex-sm-column"
                          >
                            <v-switch
                              style=""
                            ...
                            >
                              <template v-slot:prepend>
                                <v-btn
                                  class="text-capitalize font-weight-light pa-0"
                                  color="primary"
                                  density="compact"
                                  @click.stop
                            ...
                            <v-switch
                              ...
                              v-if="CheckUserActions(user_actions.update)"
                              @click.stop
                              @click="_clickedDefault(s)"
                              v-model="intent_set_default"
                              :value="s.id"
                              color="primary"
                              hide-details
                            >
                            ...
                            <v-divider vertical class="pl-5"></v-divider>
                            <v-btn
                              v-if="CheckUserActions(user_actions.delete)"
                              class="text-capitalize font-weight-light"
                              color="primary"
                              density="compact"
                              @click.stop
                              @click="modalDelete(s.id, s.intent, i)"
                              variant="text"
                              >Delete</v-btn
                            >
                          </div>
                      ...
                      <v-row>
                        <v-col cols="12" md="12">
                          <v-text-field
                            ...
                            rows="1"
                            v-model="s.intent"
                            @input="_checkInput('intent_name', s.intent, i)"
                        ...
                        <v-col cols="12" md="12">
                          <v-textarea
                            ...
                            rows="1"
                            v-model="s.desc"
                            @input="_checkInput('desc', s.desc, i)"
                            :readonly="!CheckUserActions(user_actions.update)"
                          ></v-textarea>
                        </v-col>
                        <v-col cols="12" md="12" class="pt-0">
                          <v-textarea
                            ...
                            rows="7"
                            v-model="s.script"
                            @input="_checkInput('script', s.script, i)"
                            :readonly="!CheckUserActions(user_actions.update)"
                          ></v-textarea>
                        </v-col>
                      </v-row>
                      ...
                      <v-col cols="12" class="pb-0">
                        <div class="d-flex">
                          <v-btn
                            ...
                            variant="text"
                            color="primary"
                            @click="[handleAddMetrics(i), _checkInput('metrics', 'add', i)]"
                            class="text-none"
                            v-if="CheckUserActions(user_actions.update)"
                          >
                        ...
                        <v-table>
                          ...
                          <tbody>
                            <tr
                              v-for="(item, q) in selectedItem.Intents[i].data"
                              :key="q"
                            >
                              <td>
                                <v-text-field
                                  ...
                                  class="my-3"
                                  density="compact"
                                  @input="_checkInputMetrics('name', item, i, q)"
                                  :readonly="
                              ...
                              <td>
                                <v-text-field
                                  ...
                                  class="my-3"
                                  density="compact"
                                  @input="_checkInputMetrics('desc', item, i, q)"
                                  :readonly="
                                    !CheckUserActions(user_actions.update)
                                  "
                                >
                              ...
                              <td>
                                <v-text-field
                                  ...
                                  class="my-3"
                                  density="compact"
                                  @input="_checkInputMetrics('weight', item, i, q)"
                                  :readonly="
                                    !CheckUserActions(user_actions.update)
                                  "
                                >
                              ...
                              <td align="center">
                                <v-icon
                                  ...
                                  size="small"
                                  class="me-2"
                                  @click="[deleteItemMetric(i, q), _checkInput('metrics', 'delete', i)]"
                                  color="error"
                                >
                                  mdi-delete
                                </v-icon>
        ...
        <v-row class="ma-0 my-actions">
          ...
          <v-col cols="6" class="pa-0">
            <v-btn
              ...
              color="primary"
              @click="deleteIntent()"
              :disabled="progress_circle"
            >
              <v-progress-circular
                v-if="progress_circle"
                indeterminate
                size="small"
                class="mr-2"
              ></v-progress-circular>
        ...
        <v-row class="ma-0 my-actions">
          ...
          <v-col cols="6" class="pa-0">
            <v-btn
              ...
              variant="flat"
              color="primary"
              @click="savePiiActive(set_pii_intent)"
              :disabled="progress_circle"
            >
              <v-progress-circular
                v-if="progress_circle"
  ...
  <v-dialog v-model="editingPerIntent_dialog" width="800" id="delete-modal">
    <v-card>
      <v-card-title>
        ...
        <v-btn
          variant="text"
          @click="editingPerIntent_dialog = false"
          class="float-right"
        >
          <v-icon>mdi-close</v-icon>
        </v-btn>
      ...
      <v-card-text class="py-8">
        <v-row>
          <v-col cols="12" align="center">
            ...
            <p v-else>
              The Changes you have made will create an archive of the previously analyzed data. Are You Sure?
            </p>
          </v-col>
        </v-row>
      </v-card-text>
...
export default {
  ...
  methods: {
    ...
    _save_disable(index){
      ...
      if(!intent_name && !desc && !script && !metrics && !archive && !add && !remove){
    ...
    _checkInputMetrics(type, data, p_index, c_index) {
      ...
      const check_weight = this.selectedItem_temp_2.Intents[p_index].data[c_index].cust_sat_weight === data.cust_sat_weight
      switch (type) {
        case "name":
          if(!check_weight){
            this.save_options[p_index].archive = true;
        ...
        case "desc":
          ...
          break;
        case "weight":
          const checking2 = this.selectedItem_temp_2.Intents[p_index].data[c_index].cust_sat_weight === data.cust_sat_weight
          if(!checking2){
            this.save_options[p_index].archive = true;
          } else {
    ...
    _checkInput(type, data, index) {
      switch (type) {
        ...
        case "script":
          const find2 = this.selectedItem_temp_2.Intents[index].script.replace(/\s+/g, "") === data.replace(/\s+/g, "")
      ...
      if (res.data && res.data.result) {
        if (!res.data.data) {
          ...
          this.selectedItem.Intents.map((x) => {
            ...
            this.metrics_action.push({ // checking for metrics action, if added metrics or deleted metrics
              id: x.id,
              add: false,
              remove: false
            })
    ...
    async getDefault() {
      console.log(this.default_intent);
      const config = {
