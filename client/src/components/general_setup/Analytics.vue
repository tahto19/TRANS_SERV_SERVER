...
<v-expansion-panel-text>
  ...
  <v-col cols="12" class="pb-0">
    ...
    <v-row>
      <br />
      <v-divider></v-divider>
      <h4 class="mb-5">Transcript Highlighting</h4>
      <v-col cols="12" md="12">
        <v-combobox label="Highlight" @change="_checkInput('highlight', s.highlightConfig.data, i)"
          @keydown.enter="_checkInput('highlight', s.highlightConfig.data, i)" closable-chips chips multiple
          persistent-hint placeholder="Insert text here..." variant="outlined" v-model="s.highlightConfig.data"
          :hide-no-data="true">
        </v-combobox>
      </v-col>
    </v-row>
    <v-row>
      <br />
      <v-divider></v-divider>
      <h4 class="mb-5">Summary Prompt and Filtering</h4>
      <v-col cols="12">
        <v-textarea @input="_checkInput('summary_prompt', s.notesConfig.initial_prompt, i)"
          v-model="s.notesConfig.initial_prompt" label="Custom Prompt" variant="outlined"
          placeholder="Insert text here..."></v-textarea>
      </v-col>
      <v-col cols="12">
        <v-combobox @change="_checkInput('summary_filter', s.notesConfig.filters, i)"
          @keydown.enter="_checkInput('summary_filter', s.notesConfig.filters, i)" closable-chips chips multiple
          persistent-hint placeholder="Insert text here..." label="Filters" variant="outlined"
          v-model="s.notesConfig.filters" :hide-no-data="true">
        </v-combobox>
      </v-col>
    </v-row>
    <br />
...
export default {
  ...
  methods: {
    ...
    async SubmitSetup() {
      ...
      try {
        if (
          this.selectedItem.Intents[this.edited_intent_index].notesConfig.initial_prompt == "" &&
          this.selectedItem.Intents[this.edited_intent_index].notesConfig.filters.length <= 0
        ) {
          this.selectedItem.Intents[this.edited_intent_index].notesConfig.initial_prompt = this.default_prompt;
        }

        const data = {
          organization_id: this.org_id,
          intent_id: this.selectedItem.Intents[this.edited_intent_index].id,
          ...this.selectedItem.Intents[this.edited_intent_index].notesConfig
        };
        const config = {
    ...
    _save_disable(index) {
      ...
      const summary_prompt = this.save_options[index].replace.summary_prompt;
      const summary_filter = this.save_options[index].replace.summary_filter;
      const add = this.metrics_action[index].add;
      const remove = this.metrics_action[index].remove;
    ...
    _checkInput(type, data, index) {
      switch (type) {
        ...
        case "summary_prompt":
          ...
          const find4 = this.selectedItem_temp_2.Intents[index].notesConfig.initial_prompt === data
          if (!find4) {
            this.save_options[index].replace.summary_prompt = true;
          } else {
            this.save_options[index].replace.summary_prompt = false;
          }
          this._save_disable(index);
        ...
        case "summary_filter":
          ...
          setTimeout(() => {
            if (this.selectedItem_temp_2.Intents[index].notesConfig.filters !== data.length) {

            }
            const arr = []

            data.map(x => {
              const _filter = this.selectedItem_temp_2.Intents[index].notesConfig.filters.filter(z => z === x)
              if (_filter && _filter.length === 0) {
                arr.push(true);
              } else {
                arr.push(false);
            ...
            if (_filter2 && _filter2.length == 0) {
              this.save_options[index].replace.summary_filter = true;
            } else {
              this.save_options[index].replace.summary_filter = false;
            }
            this._save_disable(index);
          }, 100)
          break;
