                 ...
                 > -->
                          ...
                          >
                            ...
                            <v-col cols="10" class="px-0">
                              <span>Defualt Intent</span>
                            </v-col>
                          </v-row> -->

                            <!-- <v-tooltip text="Default Intent">
                            <template v-slot:activator="{ props }">
                              <v-switch v-bind="props" @click.stop v-model="intent_set_default" :value="s.intent" color="primary" hide-details="">
                          </v-switch>
                            </template>
</v-tooltip> -->
<v-btn class="text-capitalize font-weight-light" color="primary" density="compact" @click.stop
  @click="_clickedPII(s.id)" variant="text" prepend-icon="mdi-filter">PII Filter</v-btn>
<v-switch style="" v-if="CheckUserActions(user_actions.update)" @click.stop @click="_clickedDefault(s)"
  v-model="intent_set_default" :value="s.intent" color="primary" hide-details>

  <template v-slot:prepend>
                                <v-label>Set Default</v-label>
                              </template>
</v-switch>

<v-btn v-if="CheckUserActions(user_actions.delete)" class="text-capitalize font-weight-light" color="primary"
  density="compact" @click.stop @click="modalDelete(s.id, s.intent, i)" variant="text">Delete</v-btn>
</div>
</div>
</div>
...
<v-expansion-panel-text>
  ...
  <v-row>
    ...
    <v-col cols="12" md="12">
      ...
      <v-textarea label="Description" variant="outlined" rows="1" v-model="s.desc"
        :readonly="!CheckUserActions(user_actions.update)"></v-textarea>
    </v-col>
  ...
  <v-col cols="12" class="pb-0">
    <div class="d-flex">
      ...
      <v-btn elevation="0" variant="text" color="primary" @click="handleAddMetrics(i)" class="text-none"
    ...
    <v-table>
      ...
      <tbody>
        <tr v-for="(item, q) in selectedItem.Intents[i].data" :key="q">
          ...
          <td align="center">
            <v-icon v-if="CheckUserActions(user_actions.delete)" size="small" class="me-2"
              @click="deleteItemMetric(i, q)" color="error">
              mdi-delete
            </v-icon>
          </td>
        </tr>
    ...
    <div class="d-flex justify-end mt-3">
      <!-- <p class="f-14 color-light mr-5">Metrics</p> -->
      <v-btn v-if="CheckUserActions(user_actions.update)" elevation="0" color="primary" @click="_editingPerIntent(i)"
        class="text-none">
        Save</v-btn>
    </div>
  </v-col>
...
<!-- <v-card class="mx-auto my-8" max-width="100%" elevation="4">
    <v-card-text class="pa-6">
      <notif-page :org_id="org_id" @notify="notify"/>
    </v-card-text>
  </v-card> -->
      ...
      <v-row class="ma-0 my-actions">
        ...
        <v-col cols="6" class="pa-0">
          ...
          <v-btn class="rounded-0 elevation-0" width="100%" height="70" variant="flat" color="primary"
            @click="saveData" :disabled="saving_data" v-else>
...
<v-dialog v-model="alert_default_dialog" persistent width="450">
  <v-card>
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn color="error" variant="text" @click="[(alert_default_dialog = false)]">
        Close
      </v-btn>
  ...
  <v-card>
    ...
    <v-card-text class="py-8">
      <v-row>
        <v-col cols="12" align="center">
          <p>
            <!-- The Changes you have made might <span>create issues</span> on previously analyzed data. You can archive the
            old data or you can replace without archiving. -->
            <!-- The changes you have proposed may introduce
            <span>potential issues</span> with previously analyzed data. The
            data will be put to archive to ensure data integrity and
            facilitate future reference. -->
            The Changes you have made might create issues on previously analyzed data. You can archive the old data or
            you can replace without archiving.
          </p>
        </v-col>
      </v-row>
    </v-card-text>
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn color="green-darken-1" @click="SaveEditedData(edited_intent_index)">
        Archive Data
      </v-btn>
...
export default {
  ...
  methods: {
    ...
    _clickedPII(intent_id) {
      const filters = this.selectedItem.Intents.filter(x => x.id == intent_id)
      this.pii_dialog = true
      this.pii_details.data = filters[0].piifilter && filters[0].piifilter.data ? filters[0].piifilter.data : []
    ...
    async saveData() {
      ...
      //   console.log(this.selectedItem_temp);
      //   this.selectedItem = [...this.selectedItem, ...this.selectedItem_temp];
      //   this.dialog = false;
      //   console.log(this.selectedItem)
    },
    async getAllData() {
      //  const config = {
      //    url: "/config?groupId=" + this.$route.params.id,
      //    method: "GET",
      //  };
      //  const res = await this.api_call(config);
      //  if (res.data && res.data.result) {
      //    this.selectedItem = res.data.data[0];
      //    this.metricRange = res.data.data[0].metricRange
      ...
      if (res.data && res.data.result) {
        if (!res.data.data) {
          ...
          this.selectedItem.Intents.map((x) => {
            if (x.default) {
              this.intent_set_default = [x.intent];
              this.intent_set_default_temp = [x.intent];
            }
          });
        }
      }
