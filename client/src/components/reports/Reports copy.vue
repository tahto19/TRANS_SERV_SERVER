<template lang="">
  <h1 class="top-header">Reports</h1>
  <div id="graphs">
    ...
    <v-row>
      ...
      <v-col cols="12" md="4">
        <v-card
          class="mx-auto mt-7 px-3 pt-1 pb-1 bt-blue"
          max-width="100%"
          elevation="4"
        >
          <v-card-title
            class="d-flex justify-space-between mb-2 border-bottom-1"
          >
            <v-row>
        ...
        >
          ...
          >
            <v-row>
              ...
              <v-col cols="6">
                <span
                  class="float-right view"
                  @click="ExpandGraph('compliance')"
                  >View</span
                >
              </v-col>
            </v-row>
    ...
    <v-row>
      <v-col cols="12" md="6">
        <v-card
          class="mx-auto mt-7 px-3 pt-1 pb-1 bt-red"
          max-width="100%"
          elevation="4"
        >
          <v-card-title
          ...
          >
            <v-row>
              ...
              <v-col cols="6"
                ><span @click="ExpandGraph('ti')" class="float-right view"
                  >View</span
                ></v-col
              >
            </v-row>
        ...
        >
          <v-card-title class="mb-2 border-bottom-1">
            <v-row>
              ...
              <v-col cols="6"
                ><span @click="ExpandGraph('senti')" class="float-right view"
                  >View</span
                ></v-col
              >
            </v-row>
  ...
  <v-card
    class="mx-auto my-8 px-5 pt-1 pb-1 bt-gray"
    max-width="100%"
    elevation="4"
  >
    <v-card-text>
      <v-row>
        ...
        >
          <template v-slot:item.sentiment="{ item }">
            <p :class="item.sentiment > 'Negative' ? item.sentiment == 'Positive' ? 'highlight-green' : '' : item.sentiment == '' ? '' : 'highlight-red'">
              {{ item.sentiment }}
            </p>
          </template>
          <template v-slot:item.score="{ item }">
            <!-- <p :class="Number(item.score) <= 70 ? 't-red' : ''"> -->
            <p :class="Number(item.score) <= score_config.low ? 'highlight-red' :  Number(item.score) >= score_config.high ? 'highlight-green' : ''">
              {{ Number(item.score) }} %
            </p>
          </template>
          ...
          <template v-slot:item.call_type="{ item }">
            <!-- <p :class="Number(item.score) <= 70 ? 't-red' : ''"> -->
              ...
              <v-chip v-bind="props" color="primary" v-if="item.call_type == 'outgoing'" variant="tonal" size="default">
                <v-icon>mdi-phone-outgoing</v-icon>
              </v-chip>
              <v-chip v-bind="props" color="info" v-if="item.call_type == 'incoming'" variant="tonal" size="default">
          ...
          <template v-slot:item.compliance="{ item }">
            <p :class="Number(item.compliance) <= score_config.low ? 'highlight-red' : Number(item.compliance) >= score_config.high ? 'highlight-green' : ''">
      ...
      if (res.data.result == "success") {
        ...
        this.my_groups = this.groups.data.map((x) => {
          x.my_groups = {
            ...
            score: parseFloat(x.csatTotal) ? parseFloat(x.csatTotal).toFixed(2) : "",
            ...
            id: x.id,
            callerid: x.callerid,
            compliance: parseFloat(x.Compliance.score) ? parseFloat(x.Compliance.score) : "",
    ...
    async getTopIntents2(type = "id", start, end) {
      ...
      if (res.data.result == "success") {
        ...
        this.top_sentiments.map((x) => {
          if (x.sentiment_name && x.sentiment_name.toLowerCase() == "negative") {
