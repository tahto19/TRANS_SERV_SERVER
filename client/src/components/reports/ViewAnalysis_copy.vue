<template>
  <v-chip :to="{ name: 'reports_default', params: { id: 'total' } }" @click="destroyAll"
    variant="text" class="mb-3 px-0">
    ...
    <v-card-text>
      <v-row>
        <div class="d-flex align-center mb-1 mr-4">
          <v-btn icon="mdi-phone-outgoing" color="primary" v-if="report_info.call_type == 'outgoing'" readonly
            size="large"></v-btn>
          <v-btn icon="mdi-phone-incoming" color="info" v-if="report_info.call_type == 'incoming'" readonly
            size="large"></v-btn>
        </div>
          ...
          <div class="d-flex align-center">
            ...
            <v-col cols="12" md="5" class="pb-0">
              <div class="d-flex align-center">
                ...
                <p class="color-light">
                  {{ formatTime(report_info.StoredSpeeches[0].duration) }}
                </p>
              </div>
            </v-col>
            <v-col cols="12" md="3">
              <div class="d-flex align-center" v-if="report_info.callerid">
                ...
                <p class="color-light">{{ report_info.call_type == "incoming" ? report_info.callerid : report_info.number_dialled }}</p>
              </div>
            </v-col>
            <v-col cols="12" md="4">
              <div class="d-flex align-center">
  ...
  <v-card class="mx-auto my-7 px-3 pt-1 pb-1" max-width="100%" elevation="4">
    <v-card-text>
      <v-row class="border-1 my-2">
        <v-col cols="12" class="text-center" style="font-size: 1.3em;">{{
    report_info.IntentResults[0].main_intent.desc
  }}</v-col>
      </v-row>
      <br />
      <v-row>
        <v-col cols="12" v-if="retrieving_audio"> 
          <div class="d-flex flex-column justify-center align-center">
            <v-progress-circular color="primary" indeterminate class="mb-2"></v-progress-circular><span>Retrieving Audio</span>
          </div>
        </v-col>
        <v-col v-if="!retrieving_audio" :cols="windowWidth <= 1120 ? '12' : '1'" class="py-0 d-flex align-center">
          <h4 class="color-light">Agent</h4>
        </v-col>
        <v-col :cols="windowWidth <= 1120 ? '12' : '11'" class="pa-0 border-left">
          <div id="waveform-0"></div>
        </v-col>
        <v-divider v-if="!retrieving_audio"></v-divider>
        <v-col v-if="!retrieving_audio" :cols="windowWidth <= 1120 ? '12' : '1'" class="py-0 d-flex align-center">
        ...
        <br />
        <v-col v-if="windowWidth >= 480 && !retrieving_audio" :cols="windowWidth <= 1120 ? '12' : '1'" class="d-flex align-center pl-7 pr-0">
          <v-btn color="rgb(24,103,192)" theme="dark" icon elevation="0" @click="playMe()" class="mr-8">
            <v-icon size="large" :icon="playing ? 'mdi-pause' : 'mdi-play'"></v-icon>
          </v-btn>
        </v-col>
        <v-col v-if="!retrieving_audio" :cols="windowWidth <= 1120 ? '12' : '11'" class="d-flex align-center px-0">
          <v-slider v-if="windowWidth <= 480" thumb-label="always" @click="playMe()" hide-details=""
            v-model="audio_value" class="mx-0" color="primary" track-color="grey" min="0" :max="audioDuration">
            <template v-slot:thumb-label="{ modelValue }">
              <v-icon>{{ playing ? "mdi-pause" : "mdi-play" }}</v-icon>
            </template>
          </v-slider>
          <v-slider v-else @click="playMe()" hide-details="" v-model="audio_value" class="mx-0" color="primary"
            track-color="grey" min="0" :max="audioDuration">
          </v-slider>
        </v-col>
        <v-col  cols="12" class="mt-2">
          <div class="d-flex pointer" @click="expand = !expand">
            <v-icon class="mr-1 color-primary-light">{{ expand ? "mdi-chevron-up" : "mdi-chevron-down" }}</v-icon>
            <p class="pb-3 text-capitalize color-primary-light undeline-hover" style="font-size: 1.3em;">
              {{ expand ? "Hide Transcription" : "Show Transcription" }}
            </p>
          </div>
          <v-expand-transition width="100%">
            <v-card id="transcriptContainer" v-show="expand" elevation="0" style="max-height: 300px; overflow: auto !important;">
              <!-- <h4 v-if="newTranscript_text2 !== ''">Transcription:</h4> -->
              <div style="margin: 0 7.5%;" v-html="newTranscript_text2 !== '' ? newTranscript_text2 : newTranscript_text"></div>
            </v-card>
          </v-expand-transition>
          <!-- <div v-html="newTranscript_text2"></div> -->
          <!-- {{ testMe(audio_value) }} -->
          <!-- <template v-for="text in transcriptTimeline">
            <span style="width: fit-content" v-if="audio_value <= text.end && audio_value >= text.start"><b>{{
    text.agent_message }}</b></span>
            <span style="width: fit-content" v-else>{{ text.agent_message }}</span>
          </template> -->
  ...
  <v-row>
    <v-col cols="12" md="7">
      <v-row>
        <v-col class="d-flex" cols="12" md="5" ref="col3">
          <v-row>
            <v-col cols="12">
              <v-card class="mx-auto bt-blue" max-width="100%" elevation="4" height="100%">
                <v-card-title class="d-flex justify-space-between mb-2">
                  <v-row>
                    <v-col cols="6">Call Score</v-col>
                  </v-row>
                </v-card-title>
                <v-card-text class="d-flex align-center justify-center" style="height: calc(100% - 48.8px)">
                  <v-divider></v-divider>
                  <c-speedometer :speedometerdata="{
    name: 'csat_score',
    num: report_info.csatTotal
      ...
      : 0,
            ...
            <v-col cols="12">
              <v-card class="mx-auto" max-width="100%" elevation="4" height="100%">
                <v-card-title class="d-flex mb-2">
                  Sentiment

                  <template v-if="upperCaseFirstLetter(report_info.SentiAnylses[0].sentiment_name) == 'Negative'">
                    <span class="highlight-red text-body-1 ml-3 d-flex align-center">{{
    upperCaseFirstLetter(report_info.SentiAnylses[0].sentiment_name) }}</span>
                  </template>

                  <template v-else-if="upperCaseFirstLetter(report_info.SentiAnylses[0].sentiment_name) == 'Positive'">
                    <span class="highlight-green text-body-1 ml-3 d-flex align-center">{{
    upperCaseFirstLetter(report_info.SentiAnylses[0].sentiment_name) }}</span>
                  </template>

                  <template v-else>
                    <span class="text-body-1 ml-3 d-flex align-center">{{
    upperCaseFirstLetter(report_info.SentiAnylses[0].sentiment_name) }}</span>
                  </template>
                </v-card-title>
    ...
    report_info.SentiAnylses[0].explanation
      ...
      num: report_info.Compliance.score
        ...
        <v-col class="d-flex" cols="12" md="7" ref="col1">
          <v-row>
            <v-col cols="12">
              <v-card max-width="100%" elevation="4" height="100%">
                <v-card-title class="d-flex justify-space-between mb-2">
                  <v-row>
                ...
                <v-card-text height="100%">
                  <v-divider></v-divider>
                  <br />
                  <div v-for="(v, n) in report_info.KpiAnylses" class="px-0 py-1 pb-1">
                    <div class="qmb border-1">
                      <div class="number">
                        <span>{{ v.rating.replace("undefined", "0").replace("%", "") + "%" }}</span>
                      </div>
                      <div>
                        <h4 class="color-light mr-3">
                          {{ v.kpi }}
    ...
    <v-col cols="12" md="5" ref="col2">
      <v-card v-if="show_col" class="px-3 pb-1 d-flex" style="min-height: 440px" height="100%" elevation="4">
        <v-card-text height="100%">
          <!-- <v-row class="border-1 my-2">
    ...
    report_info.IntentResults[0].main_intent.desc
          ...
          <v-tabs v-model="tab2" color="#494949" align-tabs="start" slider-color="#0074e4" border="bottom">
            <!-- <v-tab :value="3" class="text-capitalize">Transcript</v-tab> -->
            <v-tab :value="2" class="text-capitalize">Summary</v-tab>
          </v-tabs>
          <v-window v-model="tab2" class="analysis_sd" style="height: calc(100% - 93.6px)">
            <v-window-item :value="2" height="100%">
              <br />
              <v-sheet class="overflow-auto">
                <v-textarea label="" variant="outlined" class="overflow-auto" hide-details="auto" rows="12"
                  row-height="10" v-model="report_info.note.notes">
                </v-textarea>
              </v-sheet>
              <v-btn elevation="0" @click="saveNote()" color="rgb(0, 116, 228)" variant="flat"
            ...
            <!-- <v-window-item :value="3">
              <br />
              <v-sheet class="overflow-auto pr-5" height="100%">
                <div class="agent-box">
                  <div class="wrap">
                    <div class="message" v-html="newTranscript_text"></div>
                  </div>
                </div>
              </v-sheet>
            </v-window-item> -->
  ...
  <v-dialog v-model="dialog" v-if="view_text == 'intent_analysis'" scrollable persistent width="950">
    <v-card>
      ...
      <v-card-text class="px-6">
        <v-row>
          ...
          <v-col cols="9">
            <v-text-field v-model="agent_info.intent[0].main_intent" hide-details="auto" variant="outlined"
          ...
          <v-col cols="9">
            <v-textarea v-model="agent_info.intent[0].description" hide-details="auto" variant="outlined"
  ...
  <v-dialog v-model="dialog" v-if="view_text == 'sentiment_analysis'" scrollable persistent width="950">
    <v-card>
      <v-toolbar dark color="white" class="sticky">
      ...
      <v-card-text class="px-6">
        <v-row>
          ...
          <v-col cols="9">
            <v-text-field v-model="agent_info.sentiment[0].sentiment" hide-details="auto" variant="outlined"
              density="compact"></v-text-field>
          </v-col>
          <v-col cols="3">
            <p class="f-14 color-light">Explanation</p>
          ...
          <v-col cols="9">
            <v-textarea v-model="agent_info.sentiment[0].description" hide-details="auto" variant="outlined"
              density="compact" rows="3"></v-textarea>
          </v-col>
        </v-row>
      </v-card-text>
      ...
      <v-card-actions class="pa-3">
        <v-spacer></v-spacer>
        <v-btn color="rgb(0, 116, 228)" variant="flat" @click="saveData" class="text-none">
          Save</v-btn>
        <v-btn color="rgb(0, 116, 228)" variant="flat" class="text-none" @click="dialog = false">
          Close
        </v-btn>
...
export default {
  ...
  async created() {
    ...
    const reload = useAppStore().reload_view
    if (reload) {
      useAppStore().viewAnalysisReload(false);
  ...
  watch: {
    audio_value(val) {
      ...
      this._transcriptOnplay(val)
    },
  },
  methods: {
    _transcriptOnplay(val) {
      this.newTranscript_text2 = ""
      this.transcriptTimeline.map((x, i) => {
        if (this.transcriptTimeline[i + 1]) {
          const num = this.transcriptTimeline[i + 1].start - x.end
          const _check = val <= x.end && val >= x.start
          const newText = _check ? "<b>" + x.agent_message + "</b>" : x.agent_message

          if (num >= 2) {
            this.newTranscript_text2 += "<br><br>" + newText
          } else {
            this.newTranscript_text2 += newText
          }
        }
        // if(val <= this.autoScroll.limit){

        // } else {
        //   var container = document.getElementById("transcriptContainer");
        //   container.scrollTop = 0
        //   container.scrollTop = this.autoScroll.scroll_value;
        //   this.autoScroll.scroll_value += 200
        //   this.autoScroll.limit += 137
        // }

      })
        this.highlightText.map(x => {
            console.log(x)
            this.newTranscript_text2 = this.newTranscript_text2.replace(new RegExp('\\b' + x.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + "('?s)?\\b", 'gi'), "<span class='highlight-bg'>$&</span>");
          })
    },
    testMe(data) {
      console.log(data)
    ...
    async getReportDetails() {
      ...
      if (this.report_info.note.notesDetails.length > 0) {
        let text = "";
        this.report_info.note.notesDetails.map(x => {
          text += `${x.type}: ${x.result}\n`
          return x;
        })

        if (this.report_info.note.notes !== "" && this.report_info.note.notesDetails.length > 0) {
          this.report_info.note.notes = this.report_info.note.notes
        } else {
          this.report_info.note.notes = text
        }
      }

      this.newTranscript_text = this.report_info.content
      console.log(this.report_info)

    },
    async getHighlightTranscript(){
      try {
        const config = {
          url: "/config//HighlightTranscript",
          data: {
            id: this.$route.params.id
          },
          method: "POST",
        ...
        if (res.data.data && res.data.data.length > 0) {
          this.highlightText = res.data.data;
          res.data.data.map(x => {
            console.log(x)
            this.newTranscript_text = this.newTranscript_text.replace(new RegExp('\\b' + x.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + "('?s)?\\b", 'gi'), "<span class='highlight-bg'>$&</span>");
          })
        }
      } catch (error) {
        console.log(error)
    ...
    async getSeparate() {
      try {
        const config = {
          url: "/transcript-details/getSeperation?id=" + this.$route.params.id,
          method: "GET",
        };
        const res = await this.api_call(config);
        if (res.data.data && res.data.data.length > 0) {
          this.newTranscript_text = ""
          this.transcriptTimeline = res.data.data
          res.data.data.map((x, i) => {
            if (res.data.data[i + 1]) {
              const num = res.data.data[i + 1].start - x.end
              if (num >= 2) {
                this.newTranscript_text += "<br><br>" + x.agent_message
                // this.newTranscript_text += "<br><br>" + x.agent_message + res.data.data[i + 1].agent_message
              } else {
                this.newTranscript_text += x.agent_message
              }

              // else {
              //   test += x.start+ "-"  + x.end +"\n" + x.agent_message + "\n\n" + res.data.data[i+1].start + "-" + res.data.data[i+1].end + "\n" + res.data.data[i+1].agent_message + "\n\n"
              // }
            }
  ...
  async mounted() {
    ...
    await this.getHighlightTranscript();
