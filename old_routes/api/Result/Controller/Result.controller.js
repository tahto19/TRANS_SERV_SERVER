...
export const getCSAT = async (req, res) => {
  try {
    ...
    for (let i = 0; i < r.length; i++) {
      ...
      data["csatTotal"] = csatTotal === null ? 100 : csatTotal;
      // let temp = { save: true };
...
export const getMetricsPerIntent = async (req, res) => {
  try {
    ...
    if (
      ...
      timeSearch = {};
      timeSearch["queue_date"] = { [Op.between]: [changeStart, changeEnd] };
      console.log(timeSearch);
    }
      ...
      include: [
        {
          required: true,
          model: Transcripts,
          where: timeSearch === undefined ? 1 : timeSearch,
          // attributes: ["id"],

          include: [
            { model: Groups, where: queryFind },
            {
              required: true,
              model: KpiAnylsis,
              // attributes: [
  ...
  try {
    ...
    const getIntents = await Transcripts.findAll({
      ...
      include: [
        ...
        { required: true, model: Notes, attributes: [] },
        {
          required: true,
          model: IntentResult,
          attributes: [],
